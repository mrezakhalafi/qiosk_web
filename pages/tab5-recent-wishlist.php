<?php
    ...
    $query = $dbconn->prepare("SELECT WISHLIST_PRODUCT.*, PRODUCT.*, SHOP.NAME a, SHOP.IS_VERIFIED b FROM 
                                WISHLIST_PRODUCT LEFT JOIN PRODUCT ON WISHLIST_PRODUCT.PRODUCT_CODE = PRODUCT.CODE 
                                LEFT JOIN SHOP ON PRODUCT.SHOP_CODE = SHOP.CODE WHERE FPIN = 
                                '$id_user' AND PRODUCT.CODE NOT IN ( SELECT PRODUCT_CODE FROM COLLECTION_PRODUCT WHERE COLLECTION_CODE = '" . $collection_code . "')");
    $query->execute();
...
<body class="bg-white-background" style="display:none">
    ...
    <div id="header" class="container-fluid">
        <div class="col-12">
            <div class="row align-items-center" style="background-color: #6945A5; padding: 10px 0 30px 0; padding-bottom: 55px">
                <div class="col-1">
                    <a href="tab5-collection-wishlist.php?collection_code=<?= $collection_code ?>">
    ...
    <div class="section-recent-purchase" style="border-top-right-radius: 35px; background-color: #FAFAFF">
        <div class="container">
            <p class="small-text text-grey mb-3"><b data-translate="tab5recentpurchases-4">Recent Purchases</b></p>
                ...
                <?php foreach ($recent_purchase as $recent): ?>
                    ...
                    <div class="row recent-purchase-single-product mt-3">
                            ...
                            <?php if (substr($product_image[$i], -3) == "mp4"): ?>
                               ...
                               <!-- LOOP VIDEO FILE UNTIL GET PHOTO FOR THUMBNAIL -->
								...
								<?php if ($product_image_video): ?>
								    <img src="../images/<?= $product_image_video ?>" class="recent-purchase-image">
								<?php else: ?>
							    	<video src="../images/<?= $product_image[0] ?>#t=0.5" style="object-fit: none" type="video/mp4" class="recent-purchase-image"></video>
								<?php endif; ?>

                            <?php else: ?>

                                <img src="../images/<?= $product_image[$i] ?>" class="recent-purchase-image">
                        ...
                        <div class="col-7" style="padding-left: 9px; margin-top: -7px">
                            ...
                            <div class="smallest-text text-grey mt-2"><span data-translate="tab5recentpurchases-6"> purchased on</span> <?= date_format($order_date, "d/m/Y"); ?></div>
                        </div>
            ...
            <?php else: ?>
                <p class="text-center small-text mt-5 text-grey" data-translate="tab5recentpurchases-5">Anda belum memiliki riwayat transaksi.</p>
            <?php endif; ?>
        </div>
    </div>
</body>
